...
@section('content')
          ...
          @foreach($syllabi as $syllabus)
            <div class="col">
              <article class="svx-card shadow-sm h-100 d-flex flex-column syllabus-card" tabindex="0" role="button"
                       ...
                       data-course-code="{{ $syllabus->course->code ?? '-' }}"
                       data-course-title="{{ $syllabus->course->title ?? $syllabus->title }}"
                       data-academic-year="{{ $syllabus->academic_year }}"
                ...
                <div class="svx-card-body flex-grow-1">
                  ...
                  @if($st === 'final_approval')
                  <div class="mt-3">
                    <button type="button" class="btn final-approve-btn"
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="small text-muted mb-2" id="approveSyllabusInfo"></div>
          <div>Are you sure you want to approve this syllabus?</div>
        </div>
  ...
  <div class="modal fade" id="revisionConfirmModal" tabindex="-1" aria-labelledby="revisionConfirmLabel" aria-hidden="true" data-bs-backdrop="static">
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="revisionConfirmLabel"><i class="bi bi-arrow-clockwise"></i> Confirm Return for Revision</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
          <div class="small text-muted mb-2" id="revisionSyllabusInfo"></div>
          <div>Are you sure you want to return this syllabus for revision?</div>
        </div>
        <div class="modal-footer d-flex align-items-center gap-2">
          <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
          <button type="button" class="btn revision-confirm-btn" id="revisionConfirmBtn"
                  data-syllabus-id="" data-revision-url="">
...
<style>
  ...
  /* Review / final approval action buttons – align with submit button shape/behavior */
  .review-approve-btn, .review-revise-btn, .final-approve-btn {
  ...
  /* Final-approve button: match modal approve button styling, but full-width */
  .final-approve-btn {
    background: #fff;
    color: #28a745;
  ...
  /* Return for Revision confirm modal styling */
  #revisionConfirmModal {
    --sv-bg:   #FAFAFA;
    --sv-bdr:  #E3E3E3;
  }
  #revisionConfirmModal .modal-content {
    border: 1px solid var(--sv-bdr);
    border-radius: 16px;
  }
  #revisionConfirmModal .modal-header {
    border-bottom: 1px solid var(--sv-bdr);
    background: var(--sv-bg);
  }
  #revisionConfirmModal .modal-title {
    font-weight: 600;
    font-size: 1rem;
    display: inline-flex;
    align-items: center;
  ...
  #revisionConfirmModal .btn-light {
    background: #fff;
    border: none;
    color: #6c757d;
    transition: all 0.2s ease-in-out;
  ...
  #revisionConfirmModal .btn-light:hover,
  #revisionConfirmModal .btn-light:focus {
    background: linear-gradient(135deg, rgba(220, 220, 220, 0.88), rgba(240, 240, 240, 0.46));
    backdrop-filter: blur(7px);
    -webkit-backdrop-filter: blur(7px);
    box-shadow: 0 4px 10px rgba(108, 117, 125, 0.12);
  ...
  #revisionConfirmModal .btn-light:active {
    background: linear-gradient(135deg, rgba(240, 242, 245, 0.98), rgba(255, 255, 255, 0.62));
    box-shadow: 0 1px 8px rgba(108, 117, 125, 0.16);
  }
  #revisionConfirmModal .revision-confirm-btn {
    background: #fff;
    color: #e0a800;
  ...
  #revisionConfirmModal .revision-confirm-btn:hover {
    color: #e0a800;
    border-color: #e0a800;
    box-shadow: 0 4px 10px rgba(255, 193, 7, 0.12);
    transform: translateY(-1px);
  }
  #revisionConfirmModal .revision-confirm-btn:focus {
    outline: 0;
    box-shadow: 0 0 0 0.2rem rgba(255,193,7,0.25);
  }
  #revisionConfirmModal .revision-confirm-btn:disabled {
    opacity: 0.7; cursor: not-allowed; pointer-events: none;
  }
  ...
  document.addEventListener('DOMContentLoaded', function(){
    ...
    if (gridEl) {
      gridEl.addEventListener('click', function(ev){
        ...
        if (!target) return;
        // If a button inside the card was clicked, let that handler manage it
    ...
    // Approve opens modal; final-approve button uses same flow; Revision uses its own modal
    grid.addEventListener('click', async (ev) => {
      const approveBtn = ev.target.closest && (ev.target.closest('.review-approve-btn') || ev.target.closest('.final-approve-btn'));
      ...
      const ay = card.getAttribute('data-academic-year') || '';
      const sem = card.getAttribute('data-semester') || '';
      const separator = ' \u2022 ';
      const basic = [courseCode, courseTitle].filter(Boolean).join(separator);
      const term = ay || sem ? `AY ${ay}${sem ? `${separator}${sem}` : ''}` : '';
      const syllabusInfoText = [basic, term].filter(Boolean).join(separator);
      ...
      if (approveBtn) {
        ...
        const infoEl = document.getElementById('approveSyllabusInfo');
        if (infoEl) infoEl.textContent = syllabusInfoText;
      ...
      if (reviseBtn) {
        ...
        if (!url) return;
        const modalEl = document.getElementById('revisionConfirmModal');
        const confirmBtn = document.getElementById('revisionConfirmBtn');
        if (!modalEl || !confirmBtn) return;
        confirmBtn.dataset.syllabusId = syllabusId;
        confirmBtn.dataset.revisionUrl = url;
        const infoEl = document.getElementById('revisionSyllabusInfo');
        if (infoEl) infoEl.textContent = syllabusInfoText;
        let modal = (bootstrap.Modal && bootstrap.Modal.getInstance) ? bootstrap.Modal.getInstance(modalEl) : null;
        if (!modal) { modal = new bootstrap.Modal(modalEl); }
        modal.show();
    ...
    // Modal confirm: POST return for revision
    const revisionConfirmBtn = document.getElementById('revisionConfirmBtn');
    const revisionModalEl = document.getElementById('revisionConfirmModal');
    if (revisionConfirmBtn && revisionModalEl) {
      revisionConfirmBtn.addEventListener('click', async function(){
        ...
        const syllabusId = revisionConfirmBtn.dataset.syllabusId || '';
        if (!url || !syllabusId) return;
        const token = getCsrfToken();
        try {
